...
function BlankTestContent() {
  ...
  const renderQuestionWithBlank = (question: BlankQuestion) => {
    ...
    if (parts.length === 1) return <p className="text-xl text-white mb-6">{question.question_text}</p>;
    ...
    return (
      <div className="min-h-screen bg-[#0f0f11] flex items-center justify-center p-4">
        <div className="text-center max-w-md">
          ...
          <h1 className="text-2xl font-bold text-white mb-4">Boşluk Sorusu Bulunamadı</h1>
          <p className="text-zinc-400 mb-6">Bu hafta için boşluk sorusu eklenmemiş.</p>
        ...
        <div className="relative max-w-2xl w-full">
          <div className="rounded-2xl sm:rounded-3xl bg-zinc-900/80 border border-white/10 p-6 sm:p-12 text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center">
              <Link href="/ders" className="px-6 sm:px-8 py-3 rounded-xl bg-zinc-800 text-white font-medium hover:bg-zinc-700 transition-all">Derse Dön</Link>
              <button onClick={() => { setAnswers({}); setIsFinished(false); setCurrentIndex(0); }} className="px-6 sm:px-8 py-3 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-600 text-white font-medium hover:shadow-lg">Tekrar Dene</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <header className="fixed top-0 left-0 right-0 z-50 h-[72px] bg-[#0f0f11]/95 backdrop-blur-xl border-b border-white/5">
        <div className="max-w-7xl mx-auto h-full flex items-center justify-between px-4 sm:px-8">
          <Link href="/" className="flex items-center gap-2">
            ...
            <span className="text-xl font-bold text-white hidden sm:block">Boşluk Testi</span>
          </Link>
