...
import KazanimYonetimiPanel from '@/app/src/components/admin/KazanimYonetimiPanel';
import AdminThemeToggle from '@/app/src/components/admin/AdminThemeToggle';

export const dynamic = 'force-dynamic';

export default function KazanimYonetimiPage() {
  return (
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-10 bg-card border-b border-border px-4 sm:px-6 py-3 flex items-center gap-4">
        ...
        <h1 className="font-bold text-foreground text-sm sm:text-base flex-1">Öğrenme Çıktısı ve Kazanım Yönetimi</h1>
        <AdminThemeToggle />
      </header>
      <main className="px-4 sm:px-6 py-6 sm:py-8">
        <KazanimYonetimiPanel />
      </main>
    </div>
  );
}
