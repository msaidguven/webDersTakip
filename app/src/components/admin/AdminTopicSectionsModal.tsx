...
export default function AdminTopicSectionsModal({ topicId, topicTitle, onClose }: { topicId: number; topicTitle?: string; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-2xl border border-[#2e3348] bg-[#0f1117] p-4 sm:p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <div className="text-[11px] font-extrabold tracking-[0.18em] uppercase text-[#8b90a7]">İçerik Yönetimi</div>
            {topicTitle && <h3 className="text-base font-black text-[#e8eaf0]">{topicTitle}</h3>}
          </div>
