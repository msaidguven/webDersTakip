'use client';

import { useState } from 'react';
import { createClient } from '@/utils/supabase/client';

// Google OAuth akışı bir sayfa yönlendirmesi (redirect) ile çalışır: burada oturum
// hemen oluşmaz, tarayıcı Google'a gider, kullanıcı onaylar, sonra Supabase onu
// /auth/callback route'umuza geri yollar (bkz. app/auth/callback/route.ts) — o route
// kodu oturuma çevirip asıl hedefe (redirectTo) yönlendirir.
export default function GoogleSignInButton({ redirectTo = '/' }: { redirectTo?: string }) {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  async function handleClick() {
    setLoading(true);
    setError(null);
    const supabase = createClient();
    const { error: oauthError } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        redirectTo: `${window.location.origin}/auth/callback?redirectTo=${encodeURIComponent(redirectTo)}`,
      },
    });
    if (oauthError) {
      setError('Google ile giriş başlatılamadı, lütfen tekrar deneyin');
      setLoading(false);
    }
    // Başarılıysa taraycı zaten Google'a yönlendirilir, loading state'ini sıfırlamaya gerek yok.
  }

  return (
    <div>
      <button
        type="button"
        onClick={handleClick}
        disabled={loading}
        className="w-full py-3 rounded-xl bg-surface border border-default text-default font-medium hover:bg-surface-elevated transition-all disabled:opacity-50 flex items-center justify-center gap-2.5"
      >
        <svg width="18" height="18" viewBox="0 0 18 18" aria-hidden="true">
          <path fill="#4285F4" d="M17.64 9.2c0-.64-.06-1.25-.16-1.84H9v3.48h4.84a4.14 4.14 0 0 1-1.8 2.72v2.26h2.9c1.7-1.57 2.7-3.87 2.7-6.62Z" />
          <path fill="#34A853" d="M9 18c2.43 0 4.47-.8 5.96-2.18l-2.9-2.26c-.8.54-1.83.86-3.06.86-2.35 0-4.34-1.59-5.05-3.72H.98v2.33A9 9 0 0 0 9 18Z" />
          <path fill="#FBBC05" d="M3.95 10.7A5.4 5.4 0 0 1 3.67 9c0-.59.1-1.17.28-1.7V4.97H.98A9 9 0 0 0 0 9c0 1.45.35 2.83.98 4.03l2.97-2.33Z" />
          <path fill="#EA4335" d="M9 3.58c1.32 0 2.51.45 3.44 1.35l2.58-2.58C13.46.89 11.43 0 9 0A9 9 0 0 0 .98 4.97l2.97 2.33C4.66 5.17 6.65 3.58 9 3.58Z" />
        </svg>
        {loading ? 'Yönlendiriliyor…' : 'Google ile devam et'}
      </button>
      {error && <p className="mt-2 text-sm text-red-500">{error}</p>}
    </div>
  );
}
